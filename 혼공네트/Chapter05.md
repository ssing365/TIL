...
- **API 문서**
    ...
    **: 어떤 URL로 어떤 메서드 요청을 받았을 때 서버는 어떻게 응답할 것인가? 를 정리한 문서**
...
![스크린샷 2025-04-29 오후 1.16.50.jpg](./images/Chapter05/19.jpg)

- 한국에서 접속하거나 **한국어 계정으로 특정 URL**에 접속하면 **한국어로된 웹페이지**를 볼 수 있고,
- 다른 지역에서 접속하거나 **영어 계정으로 같은 URL**에 접속하면 **영어로 된 웹페이지**를 볼 수 있다.
...
- 송수신 가능한 자원의 형태를 **표현**이라고 한다.
- **GET 메서드의 정의** 또한 자원을 습득하기 위한 메서드가 아닌 `자원의 **특정 표현**을 습득하기 위한 메서드` 이다.
- 콘텐츠 협상 관련 주요 HTTP 헤더
    - **Accept-Language** : 선호하는 언어를 나타냄
    - **Accept** : 선호하는 미디어 타입을 나타냄
    - q : 특정 표현을 얼마나 선호하는지. 범위는 0~1, 숫자가 클수록 선호하는 것이고 q가 1이면 생략 가능하다.
    - 요청메시지
        ...
        **Accept-Language: ko-KR,ko;q=0.9;en-US;q=0.8;
        ...
        - ko-KR(한국어, 한국)의 선호도(q)가 1로 가장 높다. 다음은 en-US(영어,미국; q=0.8)